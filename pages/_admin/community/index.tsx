...
const AdminCommunity: NextPage = ({ initialInquiry, ...props }: any) => {
	...
	/** APOLLO REQUESTS **/
	const [updateBoardAreticleByAdmin] = useMutation(UPDATE_BOARD_ARTICLE_BY_ADMIN);
	const [removeBoardAreticleByAdmin] = useMutation(REMOVE_BOARD_ARTICLE_BY_ADMIN);
	...
	const updateArticleHandler = async (updateData: BoardArticleUpdate) => {
		try {
			await updateBoardAreticleByAdmin({
				variables: {
					input: updateData,
				},
			});
	...
	const removeArticleHandler = async (id: string) => {
		try {
			if (await sweetConfirmAlert('are you sure to remove?')) {
				await removeBoardAreticleByAdmin({
					variables: {
						input: id,
					},
				});
